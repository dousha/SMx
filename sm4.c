...
uint8_t sm4_sbox(uint8_t x) {
	return sm4_lut[(x & 0xf0) >> 4][x & 0x0f];
}
...
void sm4_populate_rk_table(const uint32_t key[4], uint32_t rk[32]) {
	uint32_t k[4] = {key[0] ^ sm4_fk[0], key[1] ^ sm4_fk[1], key[2] ^ sm4_fk[2], key[3] ^ sm4_fk[3]};
	for (uint8_t i = 0; i < 32; i++) {
		rk[i] = sm4_get_round_key(k, i);
		#ifdef DEBUG
		printf("rk[%02d] = %08x\n", i, rk[i]);
		#endif
	}
...
void sm4_encrypt_ecb(const uint8_t *data, uint64_t length, const uint32_t *key, uint8_t *output) {
	uint32_t rk[32], next, cipher[4];
	uint64_t offset = 0;
	sm4_populate_rk_table(key, rk);
	while (offset < length) {
		for (uint8_t i = 0; i < 4; i++) {
			cipher[i] = sm4_assemble_word(data, length, offset + 4 * i);
		}
		for (uint8_t i = 0; i < 32; i++) {
			next = sm4_f(cipher[0], cipher[1], cipher[2], cipher[3], rk[i]);
...
void sm4_decrypt_ecb(const uint8_t *data, uint64_t length, const uint32_t *key, uint8_t *output) {
	...
	while (offset < length) {
		...
		for (uint8_t i = 0; i < 32; i++) {
			next = sm4_f(cipher[0], cipher[1], cipher[2], cipher[3], rk[31 - i]);
			for (uint8_t j = 0; j < 3; j++) {
				cipher[j] = cipher[j + 1];
			}
			cipher[3] = next;
